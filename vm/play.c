...
/*   Updated: 2017/09/03 15:04:51 by vrybchyc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	nbr_live(t_player **players)
{
	int		i;
	int		nbr_live;
...
static int	end(t_vm *vm, t_player **players, int cycle)
{
	...
	if (no_live_payers(players))
		return (1);
	if (nbr_live(palyers) >= NBR_LIVE || !vm->max_checks)
	{
